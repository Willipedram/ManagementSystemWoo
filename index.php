...
function decryptField(b64){
  ...
  const key = CryptoJS.SHA256(loginToken);
  const dec = CryptoJS.AES.decrypt({ciphertext:cipher}, key, {iv:iv});
...
$('#auto-config').click(function(){
   $.post('ajax.php',{action:'read_wp_config'},function(res){
     if(res.success){
       ...
       $('#db_user').val(decryptField(res.karbarDB));
       $('#db_pass').val(decryptField(res.ramzObur));
