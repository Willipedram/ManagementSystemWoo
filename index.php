 ...
 <div class="modal-dialog modal-lg">
  <div class="modal-content">
   ...
   <div class="modal-body">
    ...
    <table class="table table-striped text-end" id="userLogTable" style="direction:ltr;">
...
function showUserLogs(id, name){
  $.post('ajax.php',{action:'fetch_user_logs',id:id},function(r){
    if(r.success){
      $('#logUser').text(name);
      if(!logTable){
        logTable=$('#userLogTable').DataTable({data:r.data,searching:false,dom:'t',columns:[
          {data:'ts',render:d=>toJalali(d)},
          {data:'action'},
          {data:'ip'}
        ]});
      }else{
        logTable.clear().rows.add(r.data).draw();
      }
...
$('#dashboardTabs button[data-bs-target="#users"]').on('shown.bs.tab',function(){
 if(!usersTable){
  usersTable=$('#usersTable').DataTable({
   ajax:{url:'ajax.php',type:'POST',data:{action:'users_list'},dataSrc:'data'},
   columns:[
    {data:'id'},
    {data:'username'},
    {data:'role'},
    {data:'status'},
    {data:'created_at'},
    {data:null,render:d=>`<button class="btn btn-sm btn-info user-log" data-id="${d.id}" data-name="${d.username}">لاگ</button> <button class="btn btn-sm btn-primary edit-user" data-id="${d.id}">ویرایش</button> <button class="btn btn-sm btn-danger delete-user" data-id="${d.id}">حذف</button>`}
   ]
  });
 ...
 Swal.fire({title:'حذف کاربر؟',icon:'warning',showCancelButton:true,confirmButtonText:'بله',cancelButtonText:'خیر'}).then(res=>{
  if(res.isConfirmed){
   $.post('ajax.php',{action:'user_delete',id:id},function(r){
    if(r.success){ toastr.success('حذف شد'); usersTable.ajax.reload(); }
    else { toastr.error(r.message); }
   },'json');
  }
 });
