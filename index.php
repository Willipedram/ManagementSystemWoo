    ...
    <div class="tab-content p-3 border border-top-0">
      ...
      <div class="tab-pane fade" id="assignManual">
        <div class="mb-3">
          ...
          <select id="assignManualSelect" multiple class="form-select" style="height:200px"></select>
        </div>
...
// Grid.js tables
let productsGrid, usersGrid, logsGrid, userLogGrid;
...
function loadAssignData(){
 fetch('ajax.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=list_categories'})
  .then(r=>r.json()).then(r=>{ if(r.success){ const sel=$('#assignCategorySelect').empty(); r.data.forEach(c=> sel.append(`<option value='${c.id}'>${c.name}</option>`)); }});
 fetch('ajax.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=unassigned_products'})
  .then(r=>r.json()).then(r=>{ if(r.success){ const sel=$('#assignManualSelect').empty(); r.data.forEach(p=> sel.append(`<option value='${p.id}'>${p.name}</option>`)); }});
}
...
initProducts(); initUsers(); initLogs(); initUserLog();
$('button[data-bs-toggle="tab"]').on('shown.bs.tab',function(e){const t=$(e.target).data('bs-target');if(t==='#products') loadProducts(); if(t==='#users') loadUsers(); if(t==='#logs') loadLogs();});
