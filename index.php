  ...
  <div class="modal-content">
   ...
   <div class="modal-body">
     <form id="editForm">
      ...
      <div class="mb-3">
        <button type="button" class="btn btn-outline-success" id="run_server_seo">تحلیل سئو</button>
        <div id="server_seo_box" class="mt-2 d-none">
          <h6>امتیاز: <span id="server_seo_score"></span></h6>
          <ul id="server_seo_details" class="list-group mb-2"></ul>
          <div class="mb-1"><strong>پیشنهاد عنوان:</strong> <span id="server_seo_title"></span></div>
          <div><strong>پیشنهاد توضیحات:</strong> <span id="server_seo_meta"></span></div>
        </div>
      </div>
      <div class="mb-3">
  ...
  fetch('ajax.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=analyze_product_seo&id='+id})
    .then(r=>r.json()).then(r=>{
      if(r.success){
        ...
        $('#server_seo_title').text(r.suggestions.title);
        $('#server_seo_meta').text(r.suggestions.meta);
        $('#server_seo_box').removeClass('d-none');
      } else {
        $('#server_seo_box').addClass('d-none');
...
$('#run_server_seo').click(function(){
  const id=$('#prod_id').val();
  if(id) loadServerSeo(id);
});
