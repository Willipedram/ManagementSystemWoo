...
<link href="https://cdn.jsdelivr.net/npm/handsontable@14.3.0/dist/handsontable.full.min.css" rel="stylesheet">
...
#priceHot{direction:rtl;}
#priceHot .htCore td{text-align:center;}
</style>
</head>
<body>
...
<div class="tab-pane fade p-3" id="analytics">
  ...
  <div class="tab-pane fade p-3" id="priceEdit">
    <div id="priceHot" class="w-100"></div>
  </div>
...
// DataTables tables
let productsTable, usersTable, logsTable, userLogDataTable, assignUsersTable, assignedProductsTable, searchConsoleTable, scChart, sessionsTable, userSessionDataTable, rolesTable, processesTable, queueTable, priceHot;
...
function initPriceTable(){
  const container=document.getElementById('priceHot');
  priceHot=new Handsontable(container,{
    data:[],
    colHeaders:['تصویر','نام محصول','قیمت'],
    columns:[
      {data:'image',readOnly:true,renderer:(inst,td,row,col,prop,value)=>{td.innerHTML=value?`<img src="${value}" style="height:40px;" loading="lazy">`:'';}},
  ...
  loadPriceData();
  priceHot.addHook('afterKeyDown',function(event){
    if(event.key==='Enter'){
      const sel=priceHot.getSelectedLast();
      if(sel){
        const row=sel[0];
        const col=sel[1];
        if(col===2){
          const rowData=priceHot.getSourceDataAtRow(row);
          const val=priceHot.getDataAtCell(row,2);
          console.log('save',rowData.id,val);
          event.preventDefault();
          priceHot.selectCell(row+1,2);
        }
      }
    }
  });
...
function loadPriceData(){
  fetch('ajax.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=list_prices'})
    .then(r=>r.json()).then(r=>{if(r.success)priceHot.loadData(r.data);});
}
