  ...
  <div class="tab-pane fade p-3" id="searchConsole">
    ...
    <div class="tab-content">
      ...
      <div class="tab-pane fade" id="scProduct">
        <button class="btn btn-primary mb-3" id="refreshProductSeo">به‌روزرسانی داده‌ها</button>
...
<div class="modal fade" id="logModal" tabindex="-1">
 <div class="modal-dialog modal-lg">
  <div class="modal-content">
   ...
   <div class="modal-body">
    <canvas id="logChart" class="mb-3"></canvas>
...
// Grid.js tables
let productsGrid, usersGrid, logsGrid, userLogGrid, assignGrid, assignProdGrid;
function seoClass(s){ if(s>=80) return 'bg-success text-white'; if(s>=50) return 'bg-warning'; if(s>0) return 'bg-danger text-white'; return 'bg-secondary text-white'; }
  ...
  .then(r=>r.json()).then(r=>{
    if(r.success){
      productsGrid.updateConfig({data:r.data.map(p=>[
        ...
        gridjs.html(`<span class='${seoClass(p.seo)}'>${p.seo}</span>`),
  ...
  .then(r=>r.json()).then(r=>{
    if(r.success && logsGrid){
      logsGrid.updateConfig({data:r.data.map(l=>[
        l.user_id,l.action,l.ip_address,l.country,l.city,l.isp,toJalali(l.timestamp)
      ])}).forceRender();
    }
  });
}
...
function loadProductSeo(){
  fetch('ajax.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=fetch_product_seo'})
   .then(r=>r.json()).then(r=>{
      if(r.success){
        if(productSeoGrid){
 ...
 fetch('ajax.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=fetch_user_logs&id='+id})
  .then(r=>r.json()).then(r=>{
    if(r.success){
      ...
      logChart=new Chart(document.getElementById('logChart'),{type:'pie',data:{labels:labels,datasets:[{data:values}]},options:{plugins:{title:{display:true,text:'توزیع ورود بر اساس کشور'}}}});
      $('#logModal').modal('show');
    }else{ toastr.error(r.message); }
  });
}
initProducts(); initUsers(); initAssignments(); initLogs(); initSearchConsole(); initProductSeo(); initUserLog();
$('button[data-bs-toggle="tab"]').on('shown.bs.tab',function(e){const t=$(e.target).data('bs-target');if(t==='#products') loadProducts(); if(t==='#users') loadUsers(); if(t==='#assignments') loadAssignUsers(); if(t==='#logs') loadLogs(); if(t==='#searchConsole'){$('#scNav button.active').data('bs-target')==='#scProduct'?loadProductSeo():loadSearchConsole();}});
$('#scNav button[data-bs-toggle="tab"]').on('shown.bs.tab',function(e){const t=$(e.target).data('bs-target'); if(t==='#scKeywords') loadSearchConsole(); if(t==='#scProduct') loadProductSeo();});
$('#refreshProductSeo').on('click',function(){NProgress.start(); fetch('ajax.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=update_product_seo'}).then(r=>r.json()).then(r=>{NProgress.done(); if(r.success){toastr.success('داده‌ها به‌روزرسانی شد'); loadProductSeo();} else toastr.error(r.message);});});
