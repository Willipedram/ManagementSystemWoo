...
// Grid.js tables
let productsGrid, usersGrid, logsGrid, userLogGrid, assignGrid, assignProdGrid, productSeoGrid, searchConsoleGrid;
...
function initSearchConsole(){
  ...
  const past=new Date(Date.now()-29*24*3600*1000).toISOString().slice(0,10);
   ...
   .then(r=>r.json()).then(r=>{
      ...
      if(r.success){
        if(!r.products.length){
          if(productSeoGrid){ productSeoGrid.updateConfig({data:[]}).forceRender(); }
          toastr.warning('داده‌ای یافت نشد');
        }else{
          if(productSeoGrid){
