...
<script>
function mswExtractAjaxError(xhr){
  ...
  if(xhr){
    var statusLine = (xhr.status || '') + ' ' + (xhr.statusText || '');
    return statusLine.trim() || 'خطای ناشناخته سرور';
  }
  return 'خطای ناشناخته سرور';
}

function mswShowCopyableError(message,title){
  var modalTitle = title || 'خطای سرور';
  Swal.fire({
    title: modalTitle,
    icon: 'error',
    ...
    html: '<div class="text-start">'
      +'<textarea id="msw-error-copy" class="form-control font-monospace" rows="6" readonly></textarea>'
      +'<button type="button" class="btn btn-outline-secondary btn-sm mt-3" id="msw-copy-btn">کپی متن خطا</button>'
      +'<div class="text-muted small mt-2">در صورت نیاز، متن را انتخاب کرده و دستی کپی کنید.</div>'
      +'</div>',
    didOpen: function(){
      var textarea = document.getElementById('msw-error-copy');
      if(textarea){
        textarea.value = message;
        textarea.focus();
        textarea.select();
      }
      var btn = document.getElementById('msw-copy-btn');
      if(btn){
        btn.addEventListener('click', function(){
          if(navigator.clipboard && navigator.clipboard.writeText){
            navigator.clipboard.writeText(message).then(function(){
              ...
              btn.textContent = 'کپی خودکار ممکن نیست';
            });
          }else if(textarea){
            textarea.focus();
            textarea.select();
   ...
   $.ajax({
     ...
     success: function(res){
       if(res.success){
         ...
         mswShowCopyableError(res.message || 'ورود نامعتبر','خطا در ورود');
       }
     },
     error: function(xhr){
       var msg = mswExtractAjaxError(xhr);
       mswShowCopyableError(msg,'خطای ورود');
     }
   });
});
</script>
...
<?php endif; ?>
 ...
 <div class="tab-pane fade p-3" id="priceEdit">
    ...
    <table id="priceTable" class="table table-striped w-100">
      <thead>
        <tr>
          <th>تصویر</th>
          <th>نام محصول</th>
...
// DataTables tables
let productsTable, usersTable, logsTable, userLogDataTable, assignUsersTable, assignedProductsTable, searchConsoleTable, scChart, rolesTable, processesTable, queueTable, priceTable;
...
function initPriceTable(){
  if($.fn.DataTable.isDataTable('#priceTable')) return;
  priceTable=$('#priceTable').DataTable({
    ajax:{url:'ajax.php',type:'POST',data:{action:'list_prices'},dataSrc:'data'},
    columns:[
      {data:'image',orderable:false,render:d=>d?`<img src="${d}" style="height:40px" loading="lazy">`:''},
      {data:'name'},
      {data:'price',orderable:false,render:(d,type,row)=>`<input type='text' class='form-control form-control-sm text-center price-input' data-id='${row.id}' value='${d}'>`}
    ],
    paging:true,
    searching:true,
    pageLength:15,
  ...
  $('#pricePageSize').on('change', function(){ priceTable.page.len(parseInt(this.value,10)).draw(); });
}

$(document).on('focus','.price-input',function(){ $(this).data('old',this.value); });
$(document).on('keydown','.price-input',function(e){
  if(e.key==='Enter'){
    const input=$(this); const id=input.data('id'); const price=input.val();
    $.post('ajax.php',{action:'update_price',id:id,price:price},function(res){
      if(res.success){ toastr.success('ذخیره شد'); }
      else{ toastr.error(res.message||'خطا'); input.val(input.data('old')); }
    },'json').fail(function(){ toastr.error('خطای ارتباط'); input.val(input.data('old')); });
