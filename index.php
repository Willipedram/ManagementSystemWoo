...
<div class="tab-pane fade p-3" id="analytics">
  ...
  <div class="tab-pane fade p-3" id="priceEdit">
    <table id="priceTable" class="table table-striped w-100"><thead><tr><th>تصویر</th><th>نام محصول</th><th>قیمت</th></tr></thead><tbody></tbody></table>
  </div>
...
// DataTables tables
let productsTable, usersTable, logsTable, userLogDataTable, assignUsersTable, assignedProductsTable, searchConsoleTable, scChart, sessionsTable, userSessionDataTable, rolesTable, processesTable, queueTable, priceTable;
...
function initPriceTable(){
  priceTable=$('#priceTable').DataTable({
    ...
    columns:[
      {data:'image',render:d=>d?`<img src="${d}" width="50" loading="lazy">`:'',orderable:false},
      {data:'name'},
      {data:'price',orderable:false,render:(d,type,row)=>`<input type="text" class="form-control price-input" data-id="${row.id}" value="${d}">`}
    ],
    paging:true,
    lengthMenu:[[10,25,50,-1],[10,25,50,'همه']],
    searching:true,
  ...
  $('#priceTable_filter input').addClass('form-control form-control-lg shadow-sm').css('width','80%');
}

$('#priceTable').on('keydown','.price-input',function(e){
  if(e.key==='Enter'){
    e.preventDefault();
    const input=$(this);
    const id=input.data('id');
    const val=input.val();
    $.post('ajax.php',{action:'update_price',id:id,price:val},function(r){
      if(r.success){
        toastr.success('ذخیره شد');
