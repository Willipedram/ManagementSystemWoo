...
case 'fetch_product_seo':
  ...
  $dateCond = "";
  if($from && $to){ $dateCond = " AND last_updated BETWEEN ? AND ?"; $paramsKw[] = $from; $paramsKw[] = $to; }
  $products = array();
  ...
  if($from && $to){
    ...
    $rest = $ldb->query("SELECT product_id,date,avg_position,ctr FROM {$lp}product_trends WHERE date>=DATE_SUB(CURDATE(),INTERVAL 30 DAY)");
    if($rest){ while($t=$rest->fetch_assoc()) $trends[]=$t; }
  }
  $db->close(); $ldb->close();
  echo json_encode(array('success'=>true,'products'=>$products,'keywords'=>$keywords,'trends'=>$trends,'coverage'=>$coverage));
  break;
