...
case 'fetch_user_logs':
  ...
  $rows = array();
  $stmt = $db->prepare("SELECT action, ip_address, timestamp FROM {$prefix}user_logs WHERE user_id=? ORDER BY id DESC LIMIT 100");
  if($stmt){
    $stmt->bind_param('i',$uid);
    $stmt->execute();
    $res=$stmt->get_result();
    while($r=$res->fetch_assoc()){ $rows[] = array('action'=>$r['action'],'ip'=>$r['ip_address'],'ts'=>$r['timestamp']); }
    $stmt->close();
  }
  $counts=array();
...
function log_event($action){
  ...
  $stmt = $db->prepare("INSERT INTO {$prefix}user_logs(user_id, action, ip_address, timestamp) VALUES (?,?,?,?)");
  if($stmt){
    $stmt->bind_param('isss',$uid,$action,$ip,$ts);
    $stmt->execute();
    $stmt->close();
  }
  $db->close();
