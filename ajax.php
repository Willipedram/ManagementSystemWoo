...
function handle_login(){
  ...
  if(!empty($schemaErrors)){
    $debug->checkpoint('schema_setup_failed',array('count'=>count($schemaErrors)));
    $db->close();
    return $debug->finalize(array('success'=>false,'message'=>'راه‌اندازی جداول سامانه با خطا مواجه شد'));
  }
...
case 'local_db_connect':
  ...
  if(!empty($schemaErrors)){
    ...
    $mysqli->close();
    echo json_encode(array('success'=>false,'message'=>'ایجاد جداول سامانه با خطا مواجه شد'));
    break;
  }
  $mysqli->close();
  echo json_encode(array('success'=>true));
...
function ensure_kpi_tables($db,$prefix,$debug=null){
  $errors = array();
  $queries = array(
    'user_kpi_events' => "CREATE TABLE IF NOT EXISTS {$prefix}user_kpi_events (
    ...
    'user_kpi_daily' => "CREATE TABLE IF NOT EXISTS {$prefix}user_kpi_daily (
    id INT AUTO_INCREMENT PRIMARY KEY,
    date DATE,
    user_id INT,
