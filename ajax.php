...
case 'unassigned_products':
  ...
  $rows = array();
  $res = $db->query($query);
  if($res){ while($r=$res->fetch_assoc()){ $rows[] = array('id'=>$r['ID'],'name'=>$r['post_title']); } }
  $db->close();
  $ldb->close();
  echo json_encode(array('success'=>true,'data'=>$rows));
  break;
...
case 'assign_manual':
  ...
  $inserted=0;
  foreach($arr as $pid){
    $stmt = $ldb->prepare("INSERT INTO {$lp}product_assignments (user_id,product_id) VALUES (?,?) ON DUPLICATE KEY UPDATE user_id=VALUES(user_id)");
    if($stmt){ $stmt->bind_param('ii',$user,$pid); if($stmt->execute()) $inserted++; $stmt->close(); }
  }
  $ldb->close();
  echo json_encode(array('success'=>true,'assigned'=>$inserted));
...
case 'list_products':
  ...
  if($perm !== 'all'){
    ...
    $ldb->close();
    if(!$ids){ echo json_encode(array('success'=>true,'data'=>array())); $db->close(); break; }
