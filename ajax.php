...
case 'fetch_product_seo':
  ...
  if($from && $to){
    $res = $ldb->query("SELECT * FROM {$lp}products_seo WHERE last_updated BETWEEN '{$from}' AND '{$to}'");
  } else {
  ...
  $sample=false;
  if(empty($products)){
    $products[]=array('product_id'=>0,'product_name'=>'نمونه','category'=>'نمونه','impressions'=>0,'clicks'=>0,'ctr'=>0,'avg_position'=>0,'indexed_status'=>'indexed');
    $sample=true;
  }
  if(empty($keywords)){
    $keywords[]=array('product_id'=>0,'keyword'=>'نمونه','clicks'=>0,'impressions'=>0,'ctr'=>0);
    $sample=true;
  }
  if(empty($trends)){
    $trends[]=array('product_id'=>0,'date'=>date('Y-m-d'),'avg_position'=>0,'ctr'=>0);
    $sample=true;
  }
  $db->close(); $ldb->close();
  echo json_encode(array('success'=>true,'products'=>$products,'keywords'=>$keywords,'trends'=>$trends,'coverage'=>$coverage,'sample'=>$sample));
  break;
...
case 'fetch_user_logs':
  ...
  $db->close();
  echo json_encode(array('success'=>true,'data'=>$rows,'counts'=>$counts,'message'=>$warn));
  break;
case 'logs_list':
  $db = connect_local();
  if(!$db){ echo json_encode(array('success'=>false,'message'=>'عدم اتصال به پایگاه داده سامانه')); break; }
  ...
  $rows = array();
  $warn='';
  $res = $db->query("SELECT id,user_id, action, ip_address, country, city, isp, timestamp FROM {$prefix}user_logs ORDER BY id DESC LIMIT 200");
  if($res){
    $key=get_setting($db,$prefix,'ipify_key');
    while($r=$res->fetch_assoc()){
