...
case 'read_wp_config':
  ...
  if(file_exists($config_path)){
    ...
    $cName = 'DB_'.'NAME';
    preg_match("/define\(\s*'$cName',\s*'([^']+)'\s*\)/", $cfg, $m); $name = isset($m[1]) ? $m[1] : '';
    $cUser = 'DB_'.'USER';
    preg_match("/define\(\s*'$cUser',\s*'([^']+)'\s*\)/", $cfg, $m); $karbar = isset($m[1]) ? $m[1] : '';
    $cPass = 'DB_'.'PASSWORD';
    preg_match("/define\(\s*'$cPass',\s*'([^']+)'\s*\)/", $cfg, $m); $ramz = isset($m[1]) ? $m[1] : '';
    $cHost = 'DB_'.'HOST';
    preg_match("/define\(\s*'$cHost',\s*'([^']+)'\s*\)/", $cfg, $m); $host = isset($m[1]) ? $m[1] : 'localhost';
    preg_match("/\$table_prefix\s*=\s*'([^']+)'/", $cfg, $m); $prefix = isset($m[1]) ? $m[1] : 'wp_';

    $plainCfg = array('host'=>$host,'name'=>$name,'user'=>$karbar,'pass'=>$ramz,'prefix'=>$prefix);
    secure_save_config($plainCfg);

    $key = hash('sha256', $_SESSION['token'], true);
    $ivU = random_bytes(16);
    $ivP = random_bytes(16);
    $encUser = base64_encode($ivU.openssl_encrypt($karbar, 'AES-256-CBC', $key, OPENSSL_RAW_DATA, $ivU));
    $encPass = base64_encode($ivP.openssl_encrypt($ramz, 'AES-256-CBC', $key, OPENSSL_RAW_DATA, $ivP));

    echo json_encode(array('success'=>true,'name'=>$name,'host'=>$host,'prefix'=>$prefix,'karbarDB'=>$encUser,'ramzObur'=>$encPass));
  } else {
...
case 'local_db_connect':
  ...
  $mysqli->query("CREATE TABLE IF NOT EXISTS {$prefix}logs (id INT AUTO_INCREMENT PRIMARY KEY, action VARCHAR(20), ip VARCHAR(45), ts DATETIME)");
  $mysqli->close();
  echo json_encode(array('success'=>true));
  break;
...
function connect_local(){
  ...
  $mysqli->set_charset('utf8mb4');
  $mysqli->query("CREATE TABLE IF NOT EXISTS {$cfg['prefix']}logs (id INT AUTO_INCREMENT PRIMARY KEY, action VARCHAR(20), ip VARCHAR(45), ts DATETIME)");
  return $mysqli;
}
