...
case 'list_prices':
  $db = connect(); if(!$db) break;
  $prefix = $_SESSION['db']['prefix'];
  $res = $db->query("SELECT p.ID,p.post_title,pr.meta_value price,im.guid image FROM {$prefix}posts p LEFT JOIN {$prefix}postmeta pr ON p.ID=pr.post_id AND pr.meta_key='_price' LEFT JOIN {$prefix}postmeta tm ON p.ID=tm.post_id AND tm.meta_key='_thumbnail_id' LEFT JOIN {$prefix}posts im ON im.ID=tm.meta_value WHERE p.post_type='product' AND p.post_status='publish'");
  $rows=array();
  if($res){
    while($r=$res->fetch_assoc()){
