...
function handle_login(){
  ...
  if(!$stmt->bind_param('s',$username)){
    $debug->checkpoint('user_query_bind_failed',array('error'=>$stmt->error));
    $stmt->close();
    $db->close();
    return $debug->finalize(array('success'=>false,'message'=>'خطای داخلی سرور'));
  }
  if(!$stmt->execute()){
    $debug->checkpoint('user_query_execute_failed',array('error'=>$stmt->error));
    $stmt->close();
    $db->close();
    return $debug->finalize(array('success'=>false,'message'=>'خطای داخلی سرور'));
  }
  $stmt->bind_result($uid,$uname,$fullName,$hash,$roleId,$permissions);
  $row = null;
  if($stmt->fetch()){
    $row = array(
      'id' => $uid,
...
case 'save_product':
  ...
  if($vres){ $vres->close(); }
  $stmtHist = $ldb->prepare("INSERT INTO {$lp}product_content_history (product_id, old_content, new_content, changed_by, changed_at, version) VALUES (?, ?, ?, NULLIF(?,0), NOW(), ?)");
  $historyId = 0;
  if($stmtHist){
...
case 'revert_content':
  ...
  if($vres){ $vres->close(); }
  $uid = intval($_SESSION['user_id']);
  $stmt2 = $ldb->prepare("INSERT INTO {$lp}product_content_history (product_id, old_content, new_content, changed_by, changed_at, version) VALUES (?, ?, ?, NULLIF(?,0), NOW(), ?)");
...
case 'sync_internal_links':
  ...
  if($idx = $db->query("SHOW INDEX FROM {$p}internal_links WHERE Key_name='uniq_category'")){
  ...
  if($cfg){
    ...
    if($wdb && !$wdb->connect_errno){
      ...
      $permalinkStructure = '';
      if($r=$wdb->query("SELECT option_value FROM {$wp}options WHERE option_name='permalink_structure' LIMIT 1")){
        $row = $r->fetch_assoc();
        $permalinkStructure = trim($row['option_value']);
        $r->close();
      }
      $categoryBase = 'product-category';
      if($r=$wdb->query("SELECT option_value FROM {$wp}options WHERE option_name='woocommerce_permalinks' LIMIT 1")){
        $row = $r->fetch_assoc();
        $raw = $row ? $row['option_value'] : '';
        if($raw !== ''){
          $decoded = @unserialize($raw);
          if(($decoded !== false || $raw === 'b:0;') && is_array($decoded) && array_key_exists('category_base',$decoded)){
            $categoryBase = trim($decoded['category_base']);
          }
        }
        $r->close();
      }
      if($categoryBase === '' && $permalinkStructure !== ''){
        $categoryBase = 'product-category';
      }
      $categories = [];
      if($res=$wdb->query("SELECT t.term_id,t.slug,t.name,tt.parent FROM {$wp}terms t JOIN {$wp}term_taxonomy tt ON t.term_id=tt.term_id WHERE tt.taxonomy='product_cat'")){
      ...
      $pathCache = [];
      $buildPath = function($termId) use (&$buildPath, &$categories, &$pathCache){
        if(isset($pathCache[$termId])) return $pathCache[$termId];
        if(!isset($categories[$termId])) return [];
        $cat = $categories[$termId];
        $segments = [];
        if($cat['parent'] && isset($categories[$cat['parent']])){
          $segments = $buildPath($cat['parent']);
        }
        $segments[] = $cat['slug'];
        $pathCache[$termId] = $segments;
        return $segments;
      };
      foreach($categories as $termId => $cat){
        $slug = $cat['slug'];
        if(isset($slugs[$slug])) continue; // avoid duplicates
        $slugs[$slug] = true;
        $eslug = $db->real_escape_string($slug);
        $title = $db->real_escape_string($cat['name']);
        $segments = $buildPath($termId);
        $encodedSegments = array_map(function($seg){ return rawurlencode($seg); }, $segments);
        if($permalinkStructure === ''){
          $url = rtrim($base,'/').'/?product_cat='.rawurlencode($slug);
        } else {
          $parts = [];
          if($categoryBase !== ''){
            $parts[] = trim($categoryBase,'/');
          }
          $parts[] = implode('/', $encodedSegments);
          $path = implode('/', array_filter($parts));
          $url = rtrim($base,'/').'/'.$path.'/';
        }
        $surl = $db->real_escape_string($url);
        $db->query("INSERT INTO {$p}internal_links(category,url,title) VALUES('$eslug','$surl','$title') ON DUPLICATE KEY UPDATE url='$surl', title='$title'");
      }
      $wdb->close();
    }
  }
...
function seed_content_history_if_empty($db,$prefix){
  ...
  if($res){
    $stmt = $db->prepare("INSERT INTO {$prefix}product_content_history (product_id, old_content, new_content, changed_by, changed_at, version) VALUES (?,NULL,?,NULL,NOW(),1)");
    while($p = $res->fetch_assoc()){
      $pid = intval($p['ID']);
