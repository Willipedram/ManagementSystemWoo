...
class ProcessManager {
  ...
  private static function runGscFetch(&$steps){
    ...
    if(!$check || $check->num_rows === 0){
      if(!$ldb->query("CREATE TABLE {$lp}search_console_daily (
        ...
        $steps[]='ایجاد جدول search_console_daily ناموفق بود: '.$ldb->error;
    ...
    if(!$stmt){
      $steps[]='آماده‌سازی درج داده‌های سرچ کنسول ناموفق بود: '.$ldb->error;
    }
    ...
    foreach($data as $row){
      ...
      $pageVal=$row['page'];
      $queryVal=$row['query'];
      $deviceVal=$row['device'];
      $countryVal=$row['country'];
      $clicksVal=intval($row['clicks']);
      $imprVal=intval($row['impressions']);
      $ctrVal=is_null($row['ctr'])?null:floatval($row['ctr']);
      $posVal=is_null($row['position'])?null:floatval($row['position']);
      $appearanceVal=$row['appearance'];

      $sess=is_null($ga['sessions'])?null:intval($ga['sessions']);
      $br=is_null($ga['bounce_rate'])?null:floatval($ga['bounce_rate']);
      $dur=is_null($ga['avg_session_duration'])?null:intval($ga['avg_session_duration']);
      $conv=is_null($ga['conversions'])?null:intval($ga['conversions']);
      $lcp=is_null($ps['lcp'])?null:floatval($ps['lcp']);
      $cls=is_null($ps['cls'])?null:floatval($ps['cls']);
      $fid=is_null($ps['fid'])?null:floatval($ps['fid']);
      $ttfb=is_null($ps['ttfb'])?null:floatval($ps['ttfb']);
      $refDom=is_null($bl['referring_domains'])?null:intval($bl['referring_domains']);
      $anchors=$bl['anchors'];
      if(is_array($anchors)){
        $anchors=json_encode($anchors,JSON_UNESCAPED_UNICODE);
      }
      $trendVal=is_null($tr)?null:intval($tr);

      if($stmt){
        $stmt->bind_param('ssssssiiddsidiiddddisi',
          $dateVal,$siteVal,$pageVal,$queryVal,$deviceVal,$countryVal,$clicksVal,$imprVal,$ctrVal,$posVal,$appearanceVal,
          $sess,$br,$dur,$conv,$lcp,$cls,$fid,$ttfb,$refDom,$anchors,$trendVal);
        if(!$stmt->execute()){
          $steps[]='ذخیره رکورد سرچ کنسول ناموفق بود: '.$stmt->error;
        }
      }
    }
    if($stmt){ $stmt->close(); }
