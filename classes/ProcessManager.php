...
class ProcessManager {
  ...
  private static function runGscFetch(&$steps){
    ...
    $payload=json_encode(array(
      'startDate'=>$from,
      'endDate'=>$to,
      // gather all dimensions needed for analysis and indexing
      'dimensions'=>array('date','page','query','device','country','searchAppearance'),
      'rowLimit'=>25000
    ));
    ...
    // ensure table exists for daily indexed metrics
    $ldb->query("CREATE TABLE IF NOT EXISTS {$lp}search_console_daily (
    ...
    foreach($rows as $r){
      $keys=$r['keys'];
      $date=$keys[0]??''; $page=$keys[1]??''; $query=$keys[2]??''; $device=$keys[3]??''; $country=$keys[4]??''; $appearance=$keys[5]??'';
      ...
      $ga=self::fetchAnalyticsMetrics($page,$date);
      $ps=self::fetchPageSpeedMetrics($page);
      $bl=self::fetchBacklinkData($page);
      $tr=self::fetchTrendsInterest($query);
      ...
      $stmt->bind_param('ssssssiiddsidiiddddisi',
        $date,$site,$page,$query,$device,$country,$clicks,$impr,$ctr,$pos,$appearance,
