<?php
function build_prompt(array $vars, array $categories = [], string $content = ''){
    $template = file_get_contents(__DIR__.'/prompt_template.txt');
    list($db,$prefix) = connect_local_prompt();
    if($db){
        $il='';
        $catNames = array_map(function($c){ return mb_strtolower($c['name']); }, $categories);
        ...
        if($res){
            while($r=$res->fetch_assoc()){
                ...
                $include = in_array($lower,$catNames) || mb_stripos($plainContent,$title)!==false;
                if($include){ $il.="<li><a href=\"{$r['url']}\">{$title}</a></li>"; }
            }
            $res->close();
        }
